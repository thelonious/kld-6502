// setup opcodes by mnemonic and address mode
let opcodes = {
...
// setup byte counts by address mode
//                ABS  ABSX ABSY ACC  IMM  IMPL IND  INDX INDY REL  ZERO ZERX ZERY
	...
	emit() {
		...
		if (this.opcode < 0) {
			if (this.mode == AddressMode.Implied) {
				...
				if (this.opcode < 0) {
					throw new Error(this.mnemonic + " does not support the following address mode: " + this.mode);
				}
			}
		}					
		...
		if (this.addressExpression !== null) {
			...
			// TODO: check for invalid address, like -1
		}					
		...
		switch (mode) {
			...
			case AddressMode.ZeroPageY:
				if (value < 0 || 0xFF < value) {
					error("Indirect address is out of range: " + value);
				}							
				result[1] = value;
				break;
			
			case AddressMode.Relative:
				var offset = value - address - 2;
				
				if (offset < -127 || 128 < offset) {
					error("Relative offset is too large: " + offset);
				}
				if (offset < 0) {
					offset = offset & 0xFF;
				}
			...
			default:
				error("Unrecognized address mode: " + mode);
		}
