...
class Assembler {
    ...
    parse(source, memory) {
        ...
        do {
            console.error("pass %s", ++pass);

            // first pass at setting label addresses
            ...
            context.address = 0;
            context.symbolChanged = false;
            ranges.clear();

            this.icode.forEach(node => {
                node.emit(context).forEach(byte => {
                    var type = (node instanceof Nodes.InstructionNode) ? "text" : "data";

                    ranges.add(context.address, type);
                    memory[context.address++] = byte;
                });
            });

            ranges.flush();
        } while (context.symbolChanged);
